...
struct PowerUpView: View {
  ...
  var body: some View {
    VStack(spacing: 16) {
      ...
      LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
        ForEach([PowerUpType.hint, .directional, .fullWord, .rotateBoard], id: \.self) {
          powerUpType in
          PowerUpButton(
            type: powerUpType,
            isActive: powerUpManager.activePowerUpType == powerUpType,
            isAvailable: powerUpManager.powerUps.first(where: { $0.type == powerUpType })?
              .isAvailable ?? false
          ) {
            handlePowerUpTap(type: powerUpType)
          }
        }
      }
      .padding(.horizontal)
  ...
  /// - Parameter type: The type of power-up tapped
  private func handlePowerUpTap(type: PowerUpType) {
    guard let viewController = hostingWindow?.rootViewController else {
      return
    }

    // If already active, clear it
    if powerUpManager.activePowerUpType == type {
      Task { @MainActor in
        powerUpManager.clearActivePowerUp()
      }
      return
